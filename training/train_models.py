...
def train_single_model(model_name, model, trainer, train_loader, val_loader, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_accuracy:
            ...
            patience_counter = 0
            # Save best model
            trainer.save_model(f"{PATHS['saved_models']}/{model_name.lower()}_model.pth")
        else:
            patience_counter += 1
...
def main():
    ...
    mlp_results = train_single_model(
        'MLP', mlp_model, mlp_trainer, train_loader, val_loader, 
        MLP_CONFIG['epochs'], device
    )
    ...
    lenet5_results = train_single_model(
        'LeNet-5', lenet5_model, lenet5_trainer, train_loader, val_loader,
        LENET5_CONFIG['epochs'], device
    )
    ...
    resnet_results = train_single_model(
        'ResNet', resnet_model, resnet_trainer, train_loader, val_loader,
        RESNET_CONFIG['epochs'], device
    )
    ...
    visualizer.plot_model_comparison(
        comparison_data,
        save_path=f"{PATHS['results']}/model_comparison.png"
    )
    
    # Confusion matrices
    visualizer.plot_confusion_matrix(
        mlp_targets, mlp_preds,
        save_path=f"{PATHS['results']}/mlp_confusion_matrix.png"
    )
    
    visualizer.plot_confusion_matrix(
        lenet5_targets, lenet5_preds,
        save_path=f"{PATHS['results']}/lenet5_confusion_matrix.png"
    )
    
    visualizer.plot_confusion_matrix(
        resnet_targets, resnet_preds,
