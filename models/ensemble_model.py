...
class EnsembleModel:
    ...
    def predict(self, image_tensor, device):
        ...
        if self.voting_method == "weighted_average":
            predicted_class, ensemble_probabilities = self._weighted_average(
                individual_predictions
            )
        elif self.voting_method == "majority_voting":
            predicted_class, ensemble_probabilities = self._majority_voting(
                individual_predictions
            )
        else:
...
class EnsembleTrainer:
    ...
    def evaluate_ensemble(self, test_loader):
        ...
        for data, target in test_loader:
            ...
            pred_class, ensemble_probs, individual_preds = ensemble_model.predict(
                data, self.device
            )
