...
def main():
    ...
    # Header
    st.markdown(
        '<h1 class="main-header">🔢 Ultimate MNIST Digit Recognition</h1>',
        unsafe_allow_html=True,
    )
        ...
        try:
            ...
            **streamlit-canvas not installed!**
            ...
            uploaded_file = st.file_uploader(
                "Upload a digit image", type=["png", "jpg", "jpeg"]
            )
        ...
        if canvas_result is not None and canvas_result.image_data is not None:
            ...
            try:
                ...
                if selected_model == "MLP" and models["mlp"] is not None:
                    ...
                    pred_class, probs = predict_lenet5(
                        models["lenet5"], preprocessed, device
                    )
                    predictions["LeNet-5"] = {
                        "class": pred_class,
                        "probabilities": probs,
                    ...
                    pred_class, probs = predict_resnet(
                        models["resnet"], preprocessed, device
                    )
                    predictions["ResNet"] = {
                        "class": pred_class,
                        "probabilities": probs,
                ...
                if predictions:
                    for model_name, pred in predictions.items():
                        with st.container():
                            st.markdown(
                                f'<div class="model-card">', unsafe_allow_html=True
                            )
                            st.markdown(f"**{model_name} Prediction:**")
                            st.markdown(
                                f"<div class='prediction-box'>", unsafe_allow_html=True
                            )
                            st.markdown(f"**Predicted Digit: {pred['class']}**")
                            st.markdown(
                                f"**Confidence: {pred['probabilities'][pred['class']]:.3f}**"
                            )
                            ...
                            digits = range(10)
                            bars = ax.bar(
                                digits, pred["probabilities"], color="skyblue"
                            )
                    ...
                    st.error(
                        "No models available for prediction. Please train the models first."
                    )
