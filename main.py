...
Examples:
  ...
  python main.py --visualize                # Generate visualizations
    ...
    parser.add_argument('--train', action='store_true',
                       help='Train all models (MLP, LeNet-5, ResNet)')
    parser.add_argument('--web', action='store_true',
                       help='Launch Streamlit web application')
    parser.add_argument('--draw', action='store_true',
                       help='Open interactive drawing canvas')
    parser.add_argument('--test', action='store_true',
                       help='Test models on sample data')
    parser.add_argument('--visualize', action='store_true',
                       help='Generate training visualizations')
    parser.add_argument('--model', choices=['mlp', 'lenet5', 'resnet', 'ensemble'],
...
def test_models(model_type=None):
    ...
    try:
        ...
        for model_name, model_key in models_to_test:
            try:
                ...
                if model_key == 'mlp':
                    ...
                    checkpoint = torch.load(f"{PATHS['saved_models']}/mlp_model.pth", map_location=device)
                elif model_key == 'lenet5':
                    model = create_lenet5_model()
                    checkpoint = torch.load(f"{PATHS['saved_models']}/lenet5_model.pth", map_location=device)
                elif model_key == 'resnet':
                    model = create_resnet_model()
                    checkpoint = torch.load(f"{PATHS['saved_models']}/resnet_model.pth", map_location=device)
        ...
        if model_type is None or model_type == 'ensemble':
            try:
                ensemble_models = {}
                for model_name, model_key in [('MLP', 'mlp'), ('LeNet-5', 'lenet5'), ('ResNet', 'resnet')]:
                    if model_key in results and results[model_name] > 0:
                        ...
                        checkpoint = torch.load(f"{PATHS['saved_models']}/{model_key}_model.pth", map_location=device)
                        ensemble_models[model_key].load_state_dict(checkpoint['model_state_dict'])
