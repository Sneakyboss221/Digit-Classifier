...
class MetricsCalculator:
    ...
    def calculate_all_metrics(y_true, y_pred):
        ...
        return {
            ...
            'precision_macro': MetricsCalculator.calculate_precision(y_true, y_pred, 'macro'),
            'precision_micro': MetricsCalculator.calculate_precision(y_true, y_pred, 'micro'),
            'precision_weighted': MetricsCalculator.calculate_precision(y_true, y_pred, 'weighted'),
            ...
            'recall_weighted': MetricsCalculator.calculate_recall(y_true, y_pred, 'weighted'),
            ...
            'confusion_matrix': MetricsCalculator.calculate_confusion_matrix(y_true, y_pred)
        }
...
def compare_models_metrics(metrics_list):
    ...
    print(f"   Precision: {best_precision['model_name']} ({best_precision['precision_macro']:.4f})")
...
def calculate_confidence_metrics(probabilities, targets):
    ...
    correct_confidence = np.mean(max_probs[correct_predictions]) if np.any(correct_predictions) else 0
    incorrect_confidence = np.mean(max_probs[~correct_predictions]) if np.any(~correct_predictions) else 0
