...
class Visualizer:
    ...
    def plot_misclassified_samples(
        self, test_loader, model, device, num_samples=16, save_path=None
    ):
    ...
    def plot_ensemble_predictions(
        ...
        predictions = [
            individual_predictions[model]["predicted_class"] for model in models
        ]
        confidences = [
            individual_predictions[model]["probabilities"][
                individual_predictions[model]["predicted_class"]
            ]
            for model in models
        ]
    ...
    def create_interactive_plot(
        ...
        fig.update_layout(
            title=f"{model_name} Training Curves", showlegend=True, height=500
        )
